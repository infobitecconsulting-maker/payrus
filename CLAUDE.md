...
**Stack:** Vite + React 19 + TypeScript, react-router-dom v7, Tailwind CSS v4, shadcn/ui (`new-york` style, Radix primitives), i18next, Convex (backend), `@usehercules/auth` (OIDC) for authentication.

**This is a UI-first demo/prototype app, not a fully wired backend app.** Convex is only used for identity — syncing the OIDC user into a `users` table and storing KYC/profile-type fields (`convex/schema.ts`, `convex/users.ts`). The 27 feature pages under `src/pages/*/page.tsx` (payments, remittance, cards, p2p, groups, gov, savings, invest, treasury, pos, etc.) are almost entirely self-contained: they render hardcoded/mock data and do not call Convex. Only `AppLayout.tsx`, `pages/auth/Callback.tsx`, and `pages/profile/page.tsx` touch `convex/react`. When extending a feature page, follow the existing pattern (local mock data shaped per profile type) unless the task explicitly calls for wiring real backend state — in which case the Convex layer (schema + functions) will need to grow from its current single-table state.
...
**Provider stack** (`src/components/providers/default.tsx`) is order-sensitive: `AuthProvider` (Hercules OIDC) → `ConvexProvider` (needs the auth context for `ConvexProviderWithHerculesAuth`) → `QueryClientProvider` → `TooltipProvider` → `ThemeProvider` → `ProfileProvider`.
