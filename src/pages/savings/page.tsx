...
const MY_POTS: SavingsPot[] = [
  { id: "1", name: "École des enfants", category: "education", target: 500000, current: 312000, currency: "XAF", monthlyContrib: 25000, interestRate: 4.5, dueDate: "Sep 2026", color: "from-violet-500 to-purple-400", icon: GraduationCap },
  { id: "2", name: "Mon petit commerce", category: "business", target: 1500000, current: 480000, currency: "XAF", monthlyContrib: 50000, interestRate: 5.2, dueDate: "Mar 2027", color: "from-primary to-emerald-400", icon: Briefcase },
  { id: "3", name: "Fonds d'urgence", category: "emergency", target: 300000, current: 300000, currency: "XAF", monthlyContrib: 0, interestRate: 3.8, dueDate: "Ongoing", color: "from-amber-500 to-yellow-400", icon: Shield },
  { id: "4", name: "Champ de maïs 2026", category: "agriculture", target: 800000, current: 155000, currency: "XAF", monthlyContrib: 40000, interestRate: 6.0, dueDate: "Dec 2026", color: "from-lime-500 to-green-400", icon: Leaf },
];

const TONTINES: TontineCircle[] = [
  { id: "1", name: "Cercle Femmes Kivu", members: 12, potAmount: 1200000, currency: "XAF", frequency: "monthly", myContrib: 100000, nextPayout: "1 Sep 2026", nextRecipient: "Fatou B.", myTurn: "Nov 2026", round: 4, totalRounds: 12, color: "bg-rose-500/15 border-rose-500/25 text-rose-400" },
  ...
  { id: "3", name: "Coopérative Bukavu", members: 20, potAmount: 4000000, currency: "CDF", frequency: "monthly", myContrib: 200000, nextPayout: "1 Oct 2026", nextRecipient: "Mwana S.", myTurn: "Jan 2027", round: 2, totalRounds: 20, color: "bg-accent/15 border-accent/25 text-accent" },
];

const INVESTMENT_PRODUCTS: InvestmentProduct[] = [
  { id: "1", name: "PayRus Épargne Plus", type: "money_market", description: "Flexible savings, withdraw anytime. Daily interest.", apy: 6.5, minAmount: 5000, currency: "XAF", duration: "Flexible", risk: "low", flag: "🌍", color: "from-primary to-emerald-400", badge: "Most Popular" },
  { id: "2", name: "Bon du Trésor BEAC 90j", type: "fixed_term", description: "BEAC-issued 90-day treasury note. Capital guaranteed.", apy: 9.2, minAmount: 50000, currency: "XAF", duration: "90 days", risk: "low", flag: "🇨🇲", color: "from-amber-500 to-yellow-400", badge: "Capital Guaranteed" },
  { id: "3", name: "Obligation Afrique Fund", type: "bond", description: "Diversified African sovereign bond portfolio.", apy: 11.4, minAmount: 100000, currency: "XAF", duration: "12 months", risk: "medium", flag: "🌍", color: "from-accent to-sky-400" },
  { id: "4", name: "Dépôt Fixe CDF 6 mois", type: "fixed_term", description: "Fixed 6-month deposit in Congolese francs.", apy: 14.0, minAmount: 200000, currency: "CDF", duration: "6 months", risk: "low", flag: "🇨🇩", color: "from-violet-500 to-purple-400" },
  { id: "5", name: "Micro-Equity Agricole", type: "micro_equity", description: "Fund farmer cooperatives. Returns tied to harvest.", apy: 18.5, minAmount: 25000, currency: "XAF", duration: "Season", risk: "high", flag: "🌱", color: "from-lime-500 to-green-400", badge: "Impact Investing" },
...
const POT_CATEGORIES: { id: SavingsPotCategory; label: string; icon: React.ComponentType<{ size?: number; className?: string }>; color: string }[] = [
  { id: "education", label: "Éducation", icon: GraduationCap, color: "from-violet-500 to-purple-400" },
  { id: "business", label: "Commerce", icon: Briefcase, color: "from-primary to-emerald-400" },
  { id: "home", label: "Logement", icon: Home, color: "from-amber-500 to-yellow-400" },
  { id: "agriculture", label: "Agriculture", icon: Leaf, color: "from-lime-500 to-green-400" },
  { id: "emergency", label: "Urgences", icon: Shield, color: "from-rose-500 to-pink-400" },
  { id: "health", label: "Santé", icon: Heart, color: "from-red-500 to-rose-400" },
  { id: "travel", label: "Voyage", icon: Globe, color: "from-accent to-sky-400" },
  { id: "custom", label: "Autre", icon: Star, color: "from-muted-foreground to-muted" },
];
    ...
    >
      {completed && (
        <div className="absolute top-3 right-3">
          <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/15 text-primary text-[10px] font-bold">
            <Trophy size={9} /> Objectif atteint!
          </span>
        </div>
      )}
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold text-foreground truncate">{pot.name}</div>
      ...
      <div className="mb-3">
        ...
        <div className="flex items-center justify-between mt-1">
          <span className={cn("text-[10px] font-semibold", completed ? "text-primary" : "text-muted-foreground")}>{pct}% complété</span>
          {pot.monthlyContrib > 0 && (
            <span className="text-[10px] text-muted-foreground">+{pot.currency} {pot.monthlyContrib.toLocaleString()}/mois</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
          ...
          <span className="text-primary font-semibold">+{pot.interestRate}%</span> rendement annuel
        </div>
        <button className="flex items-center gap-1 text-[10px] text-primary font-semibold hover:underline cursor-pointer">
          Alimenter <ArrowUpRight size={10} />
        </button>
      </div>
    </motion.div>
  );
...
function TontineCard({ t, index }: { t: TontineCircle; index: number }) {
  const isMyTurn = t.nextRecipient.startsWith("You");
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className={cn("w-9 h-9 rounded-xl border flex items-center justify-center", t.color)}>
              <Users size={15} />
            </div>
            <div>
              <div className="text-sm font-bold text-foreground">{t.name}</div>
              <div className="text-[10px] text-muted-foreground">{t.members} membres · {t.frequency === "weekly" ? "hebdomadaire" : "mensuel"}</div>
            </div>
          </div>
          {isMyTurn && (
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="bg-secondary/60 rounded-xl p-2.5 text-center">
            <div className="text-xs font-black font-mono text-foreground">{t.currency} {(t.potAmount / 1000).toFixed(0)}K</div>
            <div className="text-[9px] text-muted-foreground mt-0.5">Cagnotte totale</div>
          </div>
          <div className="bg-secondary/60 rounded-xl p-2.5 text-center">
            <div className="text-xs font-black font-mono text-foreground">{t.currency} {t.myContrib.toLocaleString()}</div>
            <div className="text-[9px] text-muted-foreground mt-0.5">Ma cotisation</div>
          </div>
          <div className="bg-secondary/60 rounded-xl p-2.5 text-center">
            <div className="text-xs font-black font-mono text-foreground">{t.round}/{t.totalRounds}</div>
            <div className="text-[9px] text-muted-foreground mt-0.5">Tour actuel</div>
          </div>
        </div>
        ...
        <div className="h-1.5 bg-secondary rounded-full overflow-hidden mb-2.5">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${(t.round / t.totalRounds) * 100}%` }}
        ...
        <div className="flex items-center justify-between text-[10px]">
          <span className="text-muted-foreground">
            Prochain: <span className="text-foreground font-semibold">{t.nextRecipient}</span> · {t.nextPayout}
          </span>
          <span className="text-muted-foreground">
            Mon tour: <span className={cn("font-semibold", isMyTurn ? "text-primary" : "text-foreground")}>{t.myTurn}</span>
          </span>
        </div>
      </div>
    </motion.div>
...
function InvestCard({ prod, index }: { prod: InvestmentProduct; index: number }) {
  const riskColor = prod.risk === "low" ? "text-primary bg-primary/10" : prod.risk === "medium" ? "text-amber-400 bg-amber-400/10" : "text-destructive bg-destructive/10";
  const riskLabel = prod.risk === "low" ? "Faible risque" : prod.risk === "medium" ? "Risque modéré" : "Risque élevé";
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      {prod.badge && (
        <div className="absolute top-3 right-3">
          <span className={cn("text-[9px] font-bold px-2 py-0.5 rounded-full", prod.badge === "Most Popular" ? "bg-primary/20 text-primary" : prod.badge === "Capital Guaranteed" ? "bg-accent/20 text-accent" : "bg-lime-500/20 text-lime-400")}>
            {prod.badge}
          </span>
        </div>
      )}
      <div className="relative">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-bold text-foreground truncate pr-16">{prod.name}</div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div>
            ...
            <div className="text-[9px] text-muted-foreground">Rendement APY</div>
          </div>
          <div>
            <div className="text-sm font-bold font-mono text-foreground">{prod.currency} {prod.minAmount.toLocaleString()}</div>
            <div className="text-[9px] text-muted-foreground">Minimum</div>
          </div>
          <div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ opacity: 0, y: 40, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40, scale: 0.96 }}
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-base font-black text-foreground">Nouvelle tirelire</h2>
                <p className="text-[11px] text-muted-foreground">Étape {step} sur 2</p>
              </div>
            ...
            {step === 1 && (
              <div className="space-y-3">
                <p className="text-xs text-muted-foreground mb-3">Choisissez l'objectif de votre épargne</p>
            ...
            {step === 2 && selectedCat && (
              <div className="space-y-3">
                <div>
                  <label className="text-[11px] text-muted-foreground font-medium mb-1.5 block">Nom de votre tirelire</label>
                  <input value={name} onChange={(e) => setName(e.target.value)} placeholder="ex: École de Marie"
                ...
                <div>
                  <label className="text-[11px] text-muted-foreground font-medium mb-1.5 block">Objectif (XAF)</label>
                ...
                <div>
                  <label className="text-[11px] text-muted-foreground font-medium mb-1.5 block">Versement mensuel automatique (XAF)</label>
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-primary/8 border border-primary/20">
                  ...
                  <p className="text-[11px] text-muted-foreground">Rendement garanti <span className="text-primary font-bold">5.2% APY</span> sur toute l'épargne accumulée.</p>
                </div>
...
const LOYALTY_POTS: LoyaltyPot[] = [
  {
    ...
    rewardOptions: [
      { label: "Free Entrée", points: 2000, icon: UtensilsCrossed },
      { label: "Dessert offert", points: 800, icon: Gift },
      { label: "Bouteille de vin", points: 4500, icon: Wine },
    ],
    monthHistory: [
      { m: "Avr", spend: 38000 }, { m: "Mai", spend: 52000 },
      { m: "Jui", spend: 41000 }, { m: "Jui", spend: 47000 }, { m: "Août", spend: 45000 },
    ],
  },
  {
    ...
    rewardOptions: [
      { label: "Entrée VIP", points: 3000, icon: Star },
      { label: "Bouteille offerte", points: 8000, icon: Wine },
      { label: "Table réservée", points: 5000, icon: Users },
    ],
    monthHistory: [
      { m: "Avr", spend: 80000 }, { m: "Mai", spend: 135000 },
      { m: "Jui", spend: 95000 }, { m: "Jui", spend: 148000 }, { m: "Août", spend: 120000 },
    ],
  },
  {
    ...
    rewardOptions: [
      { label: "Nuit offerte", points: 20000, icon: Hotel },
      { label: "Breakfast inclus", points: 5000, icon: Coffee },
      { label: "Upgrade chambre", points: 10000, icon: ArrowUpRight },
    ],
    monthHistory: [
      { m: "Avr", spend: 180000 }, { m: "Mai", spend: 0 },
      { m: "Jui", spend: 260000 }, { m: "Jui", spend: 0 }, { m: "Août", spend: 0 },
    ],
  },
  {
    ...
    rewardOptions: [
      { label: "Pinte offerte", points: 500, icon: Wine },
      { label: "Happy hour privatisé", points: 3000, icon: Users },
    ],
    monthHistory: [
      { m: "Avr", spend: 22000 }, { m: "Mai", spend: 15000 },
      { m: "Jui", spend: 19000 }, { m: "Jui", spend: 21000 }, { m: "Août", spend: 18000 },
    ],
  },
];

const EXPENSE_REPORTS: ExpenseReport[] = [
  { id: "e1", title: "Déplacement Douala–Yaoundé", date: "12 Aug 2026", amount: 85000, currency: "XAF", category: "transport", status: "approved", employee: "Kofi A.", project: "Client BDEAC" },
  { id: "e2", title: "Dîner client — Hôtel Sultani", date: "11 Aug 2026", amount: 124000, currency: "XAF", category: "meals", status: "pending", employee: "Fatou B.", project: "Pitch AfDB 2026" },
  { id: "e3", title: "Nuit hôtel Kinshasa", date: "10 Aug 2026", amount: 210000, currency: "XAF", category: "hotel", status: "approved", employee: "Amara K.", project: "Expansion RDC" },
  { id: "e4", title: "Carburant mission terrain", date: "9 Aug 2026", amount: 35000, currency: "XAF", category: "fuel", status: "rejected", employee: "Oumar D.", project: "Audit Bandundu" },
  { id: "e5", title: "Billets avion Dakar–Paris", date: "8 Aug 2026", amount: 780000, currency: "XAF", category: "transport", status: "approved", employee: "Kofi A.", project: "Roadshow Europe" },
  { id: "e6", title: "Repas équipe — séminaire", date: "7 Aug 2026", amount: 66000, currency: "XAF", category: "meals", status: "pending", employee: "Ngozi E.", project: "Team Offsite" },
];
...
const EXPENSE_CATEGORIES: Record<ExpenseReport["category"], { label: string; icon: React.ComponentType<{ size?: number; className?: string }>; color: string }> = {
  transport: { label: "Transport", icon: Plane, color: "text-blue-400" },
  meals: { label: "Repas", icon: UtensilsCrossed, color: "text-amber-400" },
  hotel: { label: "Hébergement", icon: Hotel, color: "text-cyan-400" },
  fuel: { label: "Carburant", icon: Car, color: "text-orange-400" },
  other: { label: "Autre", icon: Receipt, color: "text-muted-foreground" },
};
      ...
      <div className="p-4">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground mt-0.5">
              ...
              <span>·</span>
              <span className="text-primary font-semibold">{pot.cashbackRate}% cashback en points</span>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <div className="bg-secondary/60 rounded-xl p-2.5 text-center">
            ...
            <div className="text-[9px] text-muted-foreground">valeur cash</div>
          </div>
          <div className={cn("rounded-xl p-2.5 text-center", overBudget ? "bg-destructive/10" : "bg-secondary/60")}>
            ...
            <div className="text-[9px] text-muted-foreground">ce mois</div>
          </div>
        </div>
        ...
        {overBudget && (
          <div className="flex items-center gap-2 bg-destructive/10 border border-destructive/25 rounded-xl px-3 py-2 mb-3">
            ...
            <span className="text-[10px] text-destructive font-semibold">
              Budget dépassé de {pot.currency} {(pot.monthlySpend - pot.spendLimit).toLocaleString()} ce mois
            </span>
          </div>
        )}
        ...
        <div className="flex gap-2 overflow-x-auto pb-0.5">
          {pot.rewardOptions.map(r => (
            <button
              ...
              onClick={() => toast.success(`Récompense "${r.label}" échangée !`)}
              className={cn(
          ...
          {expanded && (
            ...
            >
              <div className="pt-4 border-t border-border mt-4 space-y-3">
                <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Historique des dépenses (XAF)</div>
                <ResponsiveContainer width="100%" height={60}>
                  <AreaChart data={pot.monthHistory} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
                    ...
                    <Tooltip formatter={(v: unknown) => [`XAF ${Number(v).toLocaleString()}`, "Dépenses"]} />
                ...
                <div className="flex items-center justify-between bg-secondary/50 rounded-xl px-3 py-2.5">
                  <div>
                    <div className="text-[10px] text-muted-foreground">Limite mensuelle</div>
                    <div className="text-xs font-bold font-mono">{pot.currency} {pot.spendLimit.toLocaleString()}</div>
                  </div>
                  <button
                    onClick={() => toast.info("Modification de la limite — bientôt disponible")}
                ...
                <button
                  onClick={() => toast.success("Solde converti en XAF et crédité sur votre wallet PayRus !")}
                ...
                >
                  <Wallet size={13} /> Convertir {pot.currency} {cashValue.toLocaleString()} en cash
                </button>
              </div>
            </motion.div>
          )}
    ...
    <div className="flex items-center gap-3 py-3 border-b border-border/60 last:border-0">
      ...
      <div className="text-right shrink-0">
        ...
        <span className={cn(
          ...
          exp.status === "pending" ? "bg-amber-400/15 text-amber-400" :
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        {alert && <AlertCircle size={14} className="text-amber-400 shrink-0" />}
      </div>
      <div className="space-y-1">
        <div className="flex justify-between text-[10px]">
          <span className={cn("font-bold font-mono", alert ? "text-amber-400" : "text-foreground")}>
        ...
        <div className="text-[9px] text-muted-foreground">{pct}% utilisé ce mois</div>
      </div>
    </motion.div>
  );
}
...
export default function SavingsPage() {
  ...
  const isCorporate = profile && ["business", "corporate", "ngo", "investment_fund", "development_bank"].includes(profile.type);
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5 pb-24 md:pb-6">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className="text-[10px] text-muted-foreground font-semibold tracking-widest uppercase">PayRus Épargne</span>
        </div>
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-black text-foreground tracking-tight">Épargne & Investissement</h1>
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-lime-500/10 border border-lime-500/20">
            <Leaf size={10} className="text-lime-400" />
            <span className="text-[10px] text-lime-400 font-semibold">Croissance Afrique</span>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 }}
        ...
        <div className="relative p-5">
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Total épargné</div>
              <div className="text-2xl font-black font-mono text-foreground tracking-tight">{currency} {(totalSaved / 1000).toFixed(0)}K</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">sur {currency} {(totalTarget / 1000).toFixed(0)}K visés</div>
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Intérêts ce mois</div>
              <div className="text-2xl font-black font-mono text-primary tracking-tight">+{currency} {totalInterest.toLocaleString()}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">rendement 5.2% APY</div>
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Tirelires actives</div>
              <div className="text-2xl font-black font-mono text-foreground tracking-tight">{MY_POTS.length}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">{MY_POTS.filter(p => p.current >= p.target).length} objectif(s) atteint(s)</div>
            </div>
          </div>
        ...
        <div className="px-5 pb-4">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-primary/15 to-lime-500/10 border border-primary/25">
            ...
            <div className="flex-1">
              <div className="text-xs font-bold text-foreground">Commencez à épargner dès XAF 500</div>
              <div className="text-[10px] text-muted-foreground">Chaque petit pas compte pour votre avenir et l'économie africaine.</div>
            </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {[
          { icon: Shield, label: t("savings.insuredSavings"), sub: "jusqu'à XAF 5M", color: "text-primary" },
          { icon: Zap, label: t("savings.dailyInterest"), sub: "crédités chaque jour", color: "text-amber-400" },
          { icon: Globe, label: "54 pays africains", sub: "réseau PayRus", color: "text-accent" },
          { icon: BookOpen, label: "Sharia-compliant", sub: "options disponibles", color: "text-lime-400" },
        ].map((b) => (
      ...
      <AnimatePresence mode="wait">
        ...
        {tab === "savings" && (
          <motion.div key="savings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-foreground">Mes tirelires ({MY_POTS.length})</h2>
              <button onClick={() => setNewPotOpen(true)}
                ...
                <Plus size={12} /> Nouvelle tirelire
              </button>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-foreground">Conseils pour épargner plus</span>
              </div>
              <div className="space-y-2">
                {[
                  { tip: "Automatisez vos versements le jour de paie pour épargner sans effort.", icon: Zap, color: "text-primary" },
                  { tip: "Divisez vos objectifs en petites étapes hebdomadaires.", icon: Target, color: "text-accent" },
                  { tip: "Invitez votre famille à co-financer vos tirelires d'éducation.", icon: Users, color: "text-lime-400" },
                ].map((item, i) => (
        ...
        {tab === "tontines" && (
          <motion.div key="tontines" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-bold text-foreground">Cercles d'épargne ({TONTINES.length})</h2>
                <p className="text-[10px] text-muted-foreground">Tontines, Njangi, ROSCA — épargne solidaire africaine</p>
              </div>
              <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 text-xs font-semibold text-primary cursor-pointer hover:bg-primary/15 transition-colors">
                <Plus size={12} /> Créer un cercle
              </button>
            </div>

            {TONTINES.map((t, i) => <TontineCard key={t.id} t={t} index={i} />)}

            {/* How it works */}
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="text-xs font-bold text-foreground mb-3">Comment fonctionne une tontine?</div>
              <div className="space-y-2.5">
                {[
                  { n: "1", text: "Formez un groupe de 5 à 30 personnes de confiance.", color: "bg-primary" },
                  { n: "2", text: "Chaque membre cotise le même montant chaque semaine ou mois.", color: "bg-accent" },
                  { n: "3", text: "À chaque cycle, un membre reçoit la totalité de la cagnotte.", color: "bg-amber-500" },
                  { n: "4", text: "PayRus sécurise les fonds et notifie tous les membres automatiquement.", color: "bg-primary" },
                ].map((s) => (
        ...
        {tab === "investments" && (
          <motion.div key="investments" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-bold text-foreground">Produits d'investissement</h2>
                <p className="text-[10px] text-muted-foreground">Faites travailler votre argent pour l'Afrique</p>
              </div>
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-secondary border border-border text-[10px] text-muted-foreground cursor-pointer hover:bg-secondary/80 transition-colors">
                <BarChart3 size={11} /> Comparer <ChevronDown size={11} />
              </div>
            </div>

            {/* Best rate highlight */}
            <div className="flex items-center gap-3 p-3.5 rounded-xl bg-gradient-to-r from-lime-500/10 to-primary/8 border border-lime-500/20">
              <div className="w-9 h-9 rounded-xl bg-lime-500/20 flex items-center justify-center shrink-0">
                <Trophy size={15} className="text-lime-400" />
              </div>
              <div className="flex-1">
                <div className="text-xs font-bold text-foreground">Meilleur rendement du moment</div>
                <div className="text-[11px] text-muted-foreground">Micro-Equity Agricole · <span className="text-lime-400 font-bold">18.5% APY</span> · Investissez dans les agriculteurs africains</div>
              </div>
            ...
            <div className="flex items-start gap-2.5 p-3.5 rounded-xl bg-secondary/60 border border-border">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                Les investissements comportent des risques. Les rendements passés ne garantissent pas les rendements futurs. PayRus est supervisé par la COBAC et la BCC. Capital garanti uniquement sur les produits signalés.
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-foreground">Impact Communautaire PayRus</span>
              </div>
              <div className="grid grid-cols-3 gap-3 text-center">
                {[
                  { v: "14,200", label: "agriculteurs financés", color: "text-lime-400" },
                  { v: "XAF 4.2B", label: "prêts accordés", color: "text-primary" },
                  { v: "94.8%", label: "taux de remboursement", color: "text-accent" },
                ].map((s) => (
        ...
        {tab === "loyalty" && (
          <motion.div key="loyalty" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Total points", value: totalLoyaltyPoints.toLocaleString(), color: "text-amber-400" },
                { label: "Valeur cash", value: `XAF ${totalLoyaltyCash.toLocaleString()}`, color: "text-primary" },
                { label: "Établissements", value: `${LOYALTY_POTS.length} liés`, color: "text-foreground" },
              ].map(s => (
            ...
            {/* How it works */}
            <div className="flex items-start gap-3 p-3.5 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-400/5 border border-amber-500/20">
              <Gift size={16} className="text-amber-400 shrink-0 mt-0.5" />
              <div>
                <div className="text-xs font-bold text-foreground mb-1">Programme de fidélité PayRus</div>
                <div className="text-[10px] text-muted-foreground leading-relaxed">
                  Chaque paiement dans un restaurant, bar, club ou hôtel partenaire vous rapporte des points. Échangez-les contre des consommations gratuites, des nuits d'hôtel ou convertissez-les en cash. Fixez un plafond mensuel pour éviter les mauvaises surprises.
                </div>
              </div>
            </div>

            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-foreground">Mes établissements ({LOYALTY_POTS.length})</h2>
              <button
                onClick={() => toast.info("Lier un nouvel établissement — bientôt disponible")}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 text-xs font-semibold text-primary cursor-pointer hover:bg-primary/15 transition-colors"
              >
                <Plus size={12} /> Lier un lieu
              </button>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-4 space-y-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold">Conseil anti-dépenses excessives</span>
              </div>
              {[
                { icon: Shield, color: "text-primary", text: "Définissez un plafond mensuel par établissement. PayRus vous alerte dès que vous l'approchez à 80%." },
                { icon: Wallet, color: "text-amber-400", text: "Convertissez vos points en cash dès que possible pour alimenter vos tirelires d'épargne." },
                { icon: Clock, color: "text-accent", text: "Les points expirent après 12 mois d'inactivité. Utilisez-les régulièrement." },
              ].map((tip, i) => (
          ...
          <motion.div key="corporate" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-card border border-border p-4">
              ...
              <div className="relative flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="text-sm font-black text-foreground">Comptes corporatifs — Voyages & Frais</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">Cartes virtuelles pré-autorisées par mission. Chaque dépense est rattachée au projet et soumise à validation en temps réel.</div>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 mt-4">
                {[
                  { label: "Frais soumis (mois)", value: `XAF ${(totalExpenses / 1000).toFixed(0)}K`, color: "text-foreground" },
                  { label: "En attente", value: `XAF ${(pendingExpenses / 1000).toFixed(0)}K`, color: "text-amber-400" },
                  { label: "Cartes actives", value: `${CORPORATE_CARDS.length}`, color: "text-primary" },
                ].map(s => (
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold">Cartes employés ({CORPORATE_CARDS.length})</h3>
                <button
                  onClick={() => toast.info("Émettre une nouvelle carte corporative — bientôt disponible")}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 text-xs font-semibold text-primary cursor-pointer hover:bg-primary/15 transition-colors"
                >
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="text-xs font-bold mb-3">Répartition des frais par catégorie</div>
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold">Notes de frais récentes</h3>
                <button
                  onClick={() => toast.info("Soumettre une note de frais — bientôt disponible")}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 text-xs font-semibold text-primary cursor-pointer hover:bg-primary/15 transition-colors"
                >
            ...
            <div className="flex items-start gap-2.5 p-3.5 rounded-xl bg-secondary/60 border border-border">
              <Shield size={13} className="text-muted-foreground mt-0.5 shrink-0" />
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                Politique de dépenses : les frais soumis dans les 7 jours de la dépense sont éligibles au remboursement. Transport aérien = accord préalable requis au-delà de XAF 300,000. Hôtels = XAF 250,000/nuit max sauf dérogation du manager.
              </p>
            </div>
          </motion.div>
        )}
